...
class Wallet{
  ...
  calculateBalance(blockchain){
    ...
    const walletInputTransaction = transactions.filter(transaction => transaction.input.address === this.publicKey);
    let startTime=0;

    if (walletInputTransaction.length > 0){
      const recentInputTransaction = walletInputTransaction.reduce(
